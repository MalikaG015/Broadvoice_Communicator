...
Then(
  'User should see the ticket successfully with subject and date as {string} subject',
  async (state) => {
    if(state==='last'){
      await searchPage.validateDataTableResultData(await baseAction.getCurrentDate(),randomEmailSubject);
    }else{
      await searchPage.validateDataTableResultData(await baseAction.getCurrentDate(),global.newSubject);
    }
  }
);

When('user returns to search tab', async () => {
