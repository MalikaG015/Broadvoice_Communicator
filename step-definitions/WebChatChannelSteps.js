...
// When('user access the webChat channel in {string} window', async (session) => {
//   await Webchat.elementClick('Webchat Channel', session);
//   counter = await WebchatCl.getWebChatCounter(false, session);
// });
When('user access the {string} in {string} window', async (menuOption,session) => {
  await Webchat.elementClick(menuOption, session);
  counter = await WebchatCl.getWebChatCounter(false, session);
});
...
When('user validate that the Average {string} Time', async(chatStatus) => {
  await WebchatCl.validateAverageFirstReplyTime(chatStatus, global.messageReceivedTime, global.chatRepliedTime, global.chatCloseTime);
});

Then(
  'user {string} the conversation with subject {string} in {string} window',
  async (status, subject, session) => {
    ...
    await WebchatCl.closeConversation(status, subject, session);
  }
);
...
// );
When(
  'user access {string} select edit option and click {string} tab',
  async (channelType,tab) => {
    await WebchatCl.clickEditAction(channelType);
    if(tab==='calendar'){
      await WebchatCl.clickCalendarTab();
    }  
...
Then('user configure an exception for today', async () => {
...
// });
Then('Then validate the ticket creation and the webchat conversation for the ticket',async()=>{
  await ticket.elementClick('Tickets');
  await ticket.clickOnLastButton();
  await ticket.selectUpdatedSort();
  await ticket.openLatestTicket();
...
Then('In the Refine assignments section select the Automatic chat delivery option', async() => {
  await WebchatCl.selectAutomaticChatDelivery();
});

Then('Set the Max concurrent chats to {string} and save the settings', async(chatCount) => {
  await WebchatCl.maxChatCount(chatCount);
});

Then('validate the static information {string} in {string} window', async(message, session) => {
  await WebchatCl.validateStaticInfo(message, session);
});

Then('verify an alert message is displayed: {string}', async(message)=>{
  await WebchatCl.verifyErrorMessage(message);
});

Then('verify that in the webchat inbox header is created a {string} after-action', async(action)=>{
  await WebchatCl.verifyAction(action);
});

Then('user approves the break request', async() => {
  await WebchatCl.approveBreakRequest();
});

When('As agent {string} the {string} conversation in {string} window', async(status, convo, session) => {
  await WebchatCl.closeAllConversation(status, convo, session);
});

When('user logout of webchat channel in {string} window', async(session)=>{
  await WebchatCl.webchatLogout(session);
});

Then('verify {string} logged out of webchat successfully in {string} window', async(userType,session)=>{
  await WebchatCl.verifyLogout(userType, session);
});

When('assign {string} to agent', async(group)=>{
  await WebchatCl.assignGroupToAgent(group);
});

Then('user click on Choose channels button', async() => {
  await WebchatCl.clickOnChooseChannel();
});

When('user select the {string} and click on save', async(channelType) => {
  await WebchatCl.selectChannel(channelType);
});

Then('user validate that the following indicators', async(tableData) => {
  const chatsDetails = tableData.rowsHash();
  await WebchatCl.validateIndicators(chatsDetails);
});

Then('user {string} the {int} conversation {string}', async(conversationStatus, convo, userContact) => {
  await WebchatCl.actionOnConversation(conversationStatus, convo, userContact);
});

Then('user back to active tab', async() => {
  await WebchatCl.backToActiveTab();
});

Then('user get the count of all indicators', async() => {
  await WebchatCl.indicatorsCount();
});