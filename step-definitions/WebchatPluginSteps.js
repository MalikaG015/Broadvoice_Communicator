...
// });
// When('user access the {string}', async (channelType) => {
//   await Webchat.elementClick(channelType);
...
When(
  ...
  async (fileName, type, message) => {
    ...
    await Webchat.FileUpload(fileName);
    await Webchat.verifyUploadMessage(type, message);
  }
);

When('validate message received successfully as file in {string} tab & {string} status',
  async (tab, status) => {
    await Webchat.verifyMessage(file, tab, status);
  }
);

When('validate message received successfully in {string} tab & {string} status',
  async (tab, status) => {
    await Webchat.verifyMessage(hashString, tab, status);
  }
);

When('user validate message received successfully in {string} tab & {string} status from day before',
  async (tab, status) => {
    await Webchat.verifyOldMessage(global.webchatOldMsg, tab, status);
  }
);

Then('user validate that conversation with {string} is now in Unread Tab', async(contactUser) => {
  await Webchat.verifyUnreadMessage(contactUser);
});
...
When(
  ...
  async (status) => {
    await Webchat.validateMessage('', status, '');
  }
);

Then('client close the first conversation', async() => {
  await Webchat.closeConversationByPlugin();
});
...
Then('client sends {string} with {string} star and {string} message', async (type, rate, message) => {
  ...
  await Webchat.SendRating(type, rate, message,comment);
});
...
When('user delete trigger action',async()=>{
  await Webchat.deleteTriggerAction();
});

When('user confirm that message with contact {string} delivered to Agent in {string} window', async(contactName, session) => {
  await Webchat.validatePluginMessage(hashString, contactName, session);
});

Then('user reply the conversation and note the time in {string} window', async (session) => {
  await Webchat.messageReply(hashStringReply, session);
});

When('user confirm that message in {string} window is not delivered', async(session) => {
  await Webchat.validateMessageDelivery(session);
});

When('user send {string} file and verify {string} text info & {string} error message',
  async (fileName, info, message) => {
    file = fileName;
    await Webchat.verifyFileUploadMessage(fileName,info, message);
  });

Then('uncheck Automatic chat delivery option in Refine assignments section',async()=>{
  await Webchat.UncheckAutomaticChatDelivery();
});

When('validate message received with {string} agent',
  async (agent) => {
    await Webchat.verifyMessageAgent(hashString, agent);
  }
);
