...
exports.Dashboard = class Dashboard extends BaseAction {
  ...
  elements = {
    ...
    voiceTabButton: '//li[@id="voice-outbound-tab"]',
    ...
    voiceInBoundTabButton: '//li[@id="voice-inbound-tab"]',
    ...
    dashboardGeneralInfoModal: '[data-translate="generalinfodasboard"]',
    realTimeToolsMenu: '[id="menu_0"]',
    campaignMessage: '.status-cell span.compound-text-with-preamble'
  };
  ...
  async navigateToDashboardPage(type='') {
    ...
    if(!(await this.isVisible(this.elements.dashboardButton))){
  ...
  async verifyAgentStateInTicketTab(state, tab) {
    ...
    if (tab === 'webchat') {
      ...
      await this.click(this.elements.webchatStateFilter);
      await this.wait(4);// added wait to update state
      await this.waitForSelector(this.elements.agentStateWebchatTab);
  ...
  async selectActionTab(type='') {    
    await this.waitForSelector(this.elements.clickAction,type);
  ...
  async forceLogoutAgent(type='') {
    await this.waitForSelector(this.elements.dashboardGeneralInfoModal, type);
  ...
  async verifystateInInVoiceTab(accessType,state,type){
    await this.wait(5); // wait for the data to load
  ...
  /**
   * Api method to get old webchat conversation
   * @param {json} body - inbound call request body
   * @returns
   */
  async webchatMsgAPI(body) {
    const headers = {
      'qa-api-key': global.QA_API_KEY,
      'Content-Type': 'application/json',
      'Accept' : 'application/json'
    };
    const response = await api.goContactHeaderPOST(global.BASE_URL + '/poll/qa/webchat/createConversation', body, headers);
    return response;
  }

  /**
   * Function to verify campaign message on Dashboard
   * @param {string} message - campaign message
   * @returns {void} nothing
 */
  async valiateCampaignMessage(message){
    await this.shouldContainText(this.elements.campaignMessage,message);
  }
};
