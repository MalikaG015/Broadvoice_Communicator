...
exports.Homepage = class Homepage extends BaseAction {
  ...
  elements = {
    ...
    simpleReporting: '//span[normalize-space()="Simple reporting"]',
    reportsDesignerPage:
      '//div[@id="default-workspace"]//span[normalize-space()="Report Designer"]',
    simpleReportingPage:
      '//div[@id="default-workspace"]//span[normalize-space()="Simple Reporting"]',
    ...
    excelTemplates: '//span[normalize-space()="Excel templates"]',
    excelTemplatesPage:
      '//div[@id="default-workspace"]//span[normalize-space()="Excel Templates"]'
  };
  ...
  async reloadPage() {
    ...
    await this.wait(5); // added wait to load the complete page
   ...
   * @return {void} Nothing
   */
  async navigateToTicketChannel(type = '') {
    await this.waitForSelector(this.elements.channelButton, type);
    await this.click(this.elements.channelButton, type);
    console.log('clicked-------------------------------------')
  }
  ...
  async logout() {
    ...
    await this.wait(2);// wait to load confirm popup
    if (await this.isVisible(this.elements.confirmLogoutButton)) {
  ...
  async verifyActiveUserList() {
    await this.wait(3); //added wait to load search result
  ...
  async openNewBrowserSessionAndVerifyUser(loginData, name) {
    ...
    let context = await browser.newContext({ viewport: null, launchOptions: { args: ['--start-maximized'] } });
  ...
  async deleteRedisCall(session) {
    ...
    if (check) {
      let value = await this.getAttributeElement(this.elements.activeCallModal, 'style', session);
  ...
  /**
   * function to call api to reset data set
   * @returns {void} nothing
   */
  async resetDataSet() {
    let body = '';
    ...
    await api.goContactPut('/poll/qa/reset/dataset', body, headers);
  }
  ...
  async navigateToReportDesigner() {
    await this.waitForSelector(this.elements.sidePanelExpandMenu);
    await this.click(this.elements.sidePanelExpandMenu);
    await this.waitForSelector(this.elements.reportsAnalyticsMenu);
    await this.click(this.elements.reportsAnalyticsMenu);
  ...
  * @return {void} Nothing
  */
  async navigateToSimpleRporting() {
    await this.waitForSelector(this.elements.sidePanelExpandMenu);
    await this.click(this.elements.sidePanelExpandMenu);
    await this.waitForSelector(this.elements.reportsAnalyticsMenu);
    await this.click(this.elements.reportsAnalyticsMenu);
    await this.waitForSelector(this.elements.simpleReporting);
    await this.click(this.elements.simpleReporting);
    await this.waitForSelector(this.elements.simpleReportingPage);
    await this.click(this.elements.reportsAnalyticsMenu);
    await this.waitForSelector(this.elements.sidePanelExpandMenu);
    await this.click(this.elements.sidePanelExpandMenu);
  }
  ...
  /**
   * function to call api to set proactive campaigns
   * @returns {void} nothing
   */
  async setProactiveCampaigns() {
    let body = '';
    body = {
      domainUuid: global.WEBCHAT_DOMAIN,
    };
    ...
    await api.goContactPut('/poll/qa/proactivecampaigns', body, headers);
  }

  /**
  * Function to navigate to excel templates
  * @return {void} Nothing
  */
  async navigateToExcelTemplates() {
    await this.waitForSelector(this.elements.sidePanelExpandMenu);
    await this.click(this.elements.sidePanelExpandMenu);
    await this.waitForSelector(this.elements.reportsAnalyticsMenu);
    await this.click(this.elements.reportsAnalyticsMenu);
    await this.waitForSelector(this.elements.excelTemplates);
    await this.click(this.elements.excelTemplates);
    await this.waitForSelector(this.elements.excelTemplatesPage);
    await this.click(this.elements.reportsAnalyticsMenu);
    await this.waitForSelector(this.elements.sidePanelExpandMenu);
    await this.click(this.elements.sidePanelExpandMenu);
  }
  /**
   * function to login to the platform as new user
   * @param {string} global.username- new user name
   * @param {string} global.password- new user password
   * @param {string} type- page type 
   * @return {void} Nothing
   */
  async loginAsNewUser(type = '') {
    await this.type(
      this.elements.usernameInput,
      global.username + '@' + global.DOMAIN,
      type
    );
    await this.type(this.elements.passwordInput, global.password, type);
    await this.click(this.elements.signInButton, type);
  }
};